...
def read_user_opcao():
    ...
    while opcao not in ['0', '1', '2', '3', '4', '5', '6']:
        opcao = input("Digite a opção (0-6): ")
    return opcao
...
def enter_ano():
    year = input("Ano de producao do veiculo: ")
    if not year:
        return None
...
def enter_automatico():
    automatico = input("O veículo é automatico (S/N): ")
    if not automatico:
        return None
    while automatico.lower() not in ['s', 'n']:
        print("Resposta inválida. Apenas 'S' or 'N'.")
        automatico = input("O veiculo é automatico (S/N): ")
...
def add_registro():
    while True:
        ...
        if placa_exists(registro['placa']):
            print(f"Erro: Já existe um cadastro com a placa {registro['placa']}")
            return      
        ...
        if not all(registro.values()):
            print("Falha ao cadastrar o veículo")
            return
        ...
        opcao = input("Gostaria de adicionar outro registro? (s/n) ")
        if opcao.lower() != 's':
            break
...
def delete_registro():
    while True:
        ...
        opcao = input("Gostaria de excluir outro registro? (s/n) ")
        if opcao.lower() != 's':
            break
...
def delete_all_registro():
    ...
    if response.status_code == 404:
        print("Não existem registros para excluir")
        return
...
def update_registro():
    while True:
        ...
        if response.status_code == 200 and response.json():
            ...
            for registro in registrados:
                print_registro(registro)
                print("Atualize os campos ou deixe vazio para manter os valores:")
            ...
            print(f"Não foi encontrado veículo com a placa {placa}")
      
        opcao = input("Gostaria de alterar outro registro? (s/n) ")
        if opcao.lower() != 's':
            break
...
def busca_placa():
    while True:
        ...
        opcao = input("Gostaria de buscar outro registro? (s/n) ")
        if opcao.lower() != 's':
            break
...
while True:
    ...
    if opcao == '0':
        ...
        listar_registros()
        opcao = input("Gostaria de retornar para a tela inicial? (s/n) ")
        if opcao.lower() != 's':
            break
    elif opcao == '2':
        busca_placa()
